...
def find_house_recursively(neighborhood: list[list[int]], row: int, col: int, solution_path: list, complete_path: list):
    ...
    #TODO make this function work - see pseudocode above 
    # In recursive function, check if a house (row, col) in the neighborhood is your house
#    (equals -2). If it is, return True.
# 2. Determine if you have already checked this house (row, col): if True, return False
# 3. Add house to solution path (list)
# 4. Recursively check the next column and current row if house (row, next_col) is even
...
#   i need to initialze the row colum
    ...
    #     if (row, next_col) in even_paths:
    #     if find_house_recursively(neighborhood, row, next_col , solution_path, complete_path):
    #         return True
    #     else: 
    #         solution_path.pop()

    # if (next_row, col) in even_paths:
    #     if find_house_recursively(neighborhood, next_row , col , solution_path, complete_path):
    #         return True
    #     else: 
    #         solution_path.pop()
